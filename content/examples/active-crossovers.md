...
Woofers and tweeters are physically offset from each other, which means the sound from each driver arrives at the listening position at slightly different times. CamillaDSP can add a small delay to the closer driver to bring the two into alignment. This is a simple `Delay` filter in the pipeline.
...
For the available filter types and configuration details, see the [CamillaDSP engine documentation](/docs/camilladsp/).
